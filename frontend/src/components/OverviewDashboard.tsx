      ...
      <div className="analytics-charts-grid overview-analytics-grid">
        <section className="glass-card analytics-chart-card compact-chart" aria-labelledby="grant-trend-title">
          ...
          {loading && !trends ? <div className="chart-loading"><LoaderCircle size={20} /> Loading grant trend…</div> : trends?.status === "available" && trends.items.length ? <>
            ...
            <div className="analytics-chart-plot compact"><ResponsiveContainer width="100%" height="100%"><BarChart data={trends.items} margin={{ top: 6, right: 8, bottom: 0, left: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(15,23,42,.08)" /><XAxis dataKey="month" minTickGap={28} tick={{ fill: "#64748b", fontSize: 10 }} tickFormatter={value => String(value).slice(2)} /><YAxis width={58} tick={{ fill: "#64748b", fontSize: 10 }} tickFormatter={value => formatCurrency(Number(value), trends.currency).replace("£", "£")} /><Tooltip content={({ active, payload: rows, label }) => { const item = rows?.[0]?.payload as TrendItem | undefined; if (!active || !item) return null; return <div className="chart-tooltip"><strong>{label}</strong>{item.coverage_status === "observed" ? <><span>{formatCurrency(item.total_amount, trends.currency)}</span><span>{item.grant_count} grants · {item.mapped_grant_count} mapped · {item.unmapped_grant_count} unmapped</span></> : <span>{item.coverage_status === "partial" ? "Source records without a valid aggregate." : "No source coverage established; not a confirmed zero."}</span>}</div>; }} /><Bar dataKey="total_amount" name="Awarded funding" fill="var(--nl-unicorn)" radius={[4, 4, 0, 0]} isAnimationActive={false} /></BarChart></ResponsiveContainer></div>
        ...
        <section className="glass-card analytics-chart-card compact-chart" aria-labelledby="programme-chart-title">
          ...
          {loading && !themes ? <div className="chart-loading"><LoaderCircle size={20} /> Loading programme allocation…</div> : themes?.status === "available" && visibleThemeItems.length ? <>
            ...
            <div className="analytics-chart-plot compact"><ResponsiveContainer width="100%" height="100%"><BarChart data={visibleThemeItems} layout="vertical" margin={{ top: 0, right: 10, bottom: 0, left: 12 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(15,23,42,.08)" /><XAxis type="number" tick={{ fill: "#64748b", fontSize: 10 }} tickFormatter={value => formatCurrency(Number(value), themes.currency).replace("£", "£")} /><YAxis type="category" width={125} dataKey="programme_area" tick={{ fill: "#475569", fontSize: 10 }} /><Tooltip formatter={(value) => formatCurrency(Number(value), themes.currency)} /><Bar dataKey="allocated_amount" radius={[0, 4, 4, 0]} isAnimationActive={false}>{visibleThemeItems.map(item => <Cell key={item.programme_area} fill={item.programme_area === "Unclassified" ? "#94a3b8" : item.programme_area === "Other" ? "#c4b5fd" : "#7c3aed"} />)}</Bar></BarChart></ResponsiveContainer></div>
