...
class TestEcbExchangeRateBackfill(unittest.TestCase):
    ...
    def test_reuses_stored_rates_and_fetches_only_uncovered_dates(self):
        ...
        self.assertEqual(fetch_windows["USD"], [("2025-02-19", "2025-03-01")])

    def test_backfill_uses_award_date_or_previous_ecb_business_day_without_overwriting_source(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result["conversion_status_counts"]["ecb_previous_business_day"], 1)
            self.assertEqual(result["overview_cache_prewarmed_grants"], 3)
            self.assertEqual(result["conversion_status_counts"]["native_eur"], 1)
            self.assertEqual(result["conversion_status_counts"]["ecb_rate_unavailable"], 1)
            ...
            self.assertEqual(rows, [
                ...
                ("pre-euro-reference", 100.0, None, None, "ecb_rate_unavailable"),
                ("weekend-gbp", 100.0, 125.0, "2024-01-05", "ecb_previous_business_day"),
            ])
