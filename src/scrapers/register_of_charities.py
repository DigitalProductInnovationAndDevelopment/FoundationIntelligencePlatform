"""Collector for the Charity Commission for England and Wales API.

Requires ``CHARITY_COMMISSION_API_KEY``. Fetches registration details, financial history,
assets and liabilities, and activity descriptions, caching raw responses so downstream
consolidation can be re-run without further API calls.

Supports resumption via already-completed registration numbers, so an interrupted run
does not re-request records it already holds. This is the authoritative source for UK
registration identity and status; its values are source facts and must never be
