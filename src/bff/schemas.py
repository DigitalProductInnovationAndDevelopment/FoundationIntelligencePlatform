...
These models *are* the contract: FastAPI derives the OpenAPI schema served at ``/docs``
from them, and ``config/golden/api-contract.json`` asserts the shapes that clients rely
on. Changing a field here changes the public API, so update the golden fixture
deliberately rather than to make a test pass.

Two conventions carry the platform's provenance discipline into the wire format:
source facts, deterministic inferences and platform-derived values live in separate
fields and are never merged; and absence is explicit (for example
``transaction_data_unavailable`` or ``organization_level_only``) so that a client can
never mistake unknown data for zero activity.
"""
